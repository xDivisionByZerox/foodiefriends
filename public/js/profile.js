...
document.querySelectorAll("#restaurant_add_btn").forEach((e, index) => {
  e.addEventListener("click", () => {
    $("#restaurantModal").modal("show");
    if (index == 0) {
      showToast(
        "Hint",
        "Add restaurants you would recommend for new friends",
        true
      );
    } else if (index == 1) {
      showToast(
        "Hint",
        "Keep in mind that the people you match with <b>depends on what restaurant you choose.</b>",
        true
      );
    }
  });
  ...
  restaurantList.forEach((e, index) => {
    ...
    if (hasFoodOrRestaurant) {
      if (
        ...
        index > 0 &&
        e.name == restaurantList[index - 1].name
      ) {
