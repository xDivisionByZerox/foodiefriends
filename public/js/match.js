...
const startOfWeek = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate()+1);
const endOfWeek = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate() + 6);
...
document.addEventListener("DOMContentLoaded", () => {
  const reSelectRestaurantBtn = document.querySelector("#restaurant_reselect_btn");
...
document.addEventListener("DOMContentLoaded",async()=>{
  ...
  checkDate().then(result => {
    ...
    result?document.querySelector("#date_notification_text").innerHTML=currentUserDate.date+" "+currentUserDate.time:document.querySelector("#date_notification_text").innerHTML="Haven't chosed one"
...
document.getElementById('datePicker').addEventListener('input', updateSelectedDate);
...
saveChangesButton.addEventListener("click", async (e) => {
  const selectedGenderLabel = document.querySelector('input[name="btnradio"]:checked + label');
...
async function like(id) {
    ...
    let match_response = await fetch('/api/matches',{
      ...
      body: JSON.stringify({ "USERA": userId,
...
function updateDateTime() {
  ...
  document.getElementById('selectedDateTime').innerText = 'Selected time: ' + selectedTime;
}
